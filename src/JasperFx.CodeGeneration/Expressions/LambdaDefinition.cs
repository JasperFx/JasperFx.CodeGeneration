...
{
    private readonly Dictionary<Variable, Expression> _variables = new();
    ...
    public TFunc Compile<TFunc>() where TFunc : class
    {
        if (!Body.Any())
        {
            throw new InvalidOperationException("There are no Body expressions for this LambdaDefinition");
        }
