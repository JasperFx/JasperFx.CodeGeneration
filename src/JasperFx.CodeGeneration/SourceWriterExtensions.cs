...
public static class SourceWriterExtensions
{
    private static readonly string
        returnCompletedTask = $"return {typeof(Task).FullName}.{nameof(Task.CompletedTask)};";
    ...
    public static void StartClass(this ISourceWriter writer, string className, params Type[] inheritsOrImplements)
    {
        ...
        {
            writer.Write(
                $"BLOCK:public class {className} : {inheritsOrImplements.Select(x => x.FullNameInCode()).Join(", ")}");
        }
    }
}